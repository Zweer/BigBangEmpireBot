package com.playata.application.ui.dialogs
{
   ...
   public class DialogGuildBattleSearch extends UiDialog
   {
      ...
      private var _attackAction:Boolean = false;
      
      private var _currentAttackGuild:Guild = null;
      ...
      {
         var _loc1_:SymbolDialogGuildBattleSearchGeneric = _vo as SymbolDialogGuildBattleSearchGeneric;
         _btnClose.dispose();
         _btnClose = null;
         ...
         switch(_loc9_)
         {
            case "getGuildBattleList":
               if(param1.error == "")
               {
                  _guilds = new Vector.<GuildBattleSearchResult>();
                  _loc9_ = 0;
                  var _loc8_:* = param1.appResponse.guild_battle_entries;
            ...
            case "getGuild":
               if(param1.error == "")
               {
                  ...
                  _loc7_ = Guild.fromData(param1.data);
                  if(_attackAction)
                  {
                     _currentAttackGuild = _loc7_;
                     Environment.application.sendActionRequest("checkAttackGuild",{
                        "guild_id":_currentAttackGuild.id,
                        "time":1
                     },handleRequests);
                  }
                  else
                  {
                     ViewManager.instance.showGuild(_loc7_);
                     close();
                  }
               }
               else
            ...
            case "checkAttackGuild":
               if(param1.error == "")
               {
                  Environment.application.updateData(param1.data);
                  close();
                  Environment.panelManager.showDialog(new DialogGuildBattleCreate(_currentAttackGuild));
               }
