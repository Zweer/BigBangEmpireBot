package com.playata.application.ui.elements.slotmachine
{
   ...
   public class UiSlotmachineChat
   {
      ...
      private static var _right:int = 670;
      
      private static var _normal:int = 270;
      
      private static var _left:int = -130;
      ...
      private var _messages:Vector.<SlotmachineLogMessage>;
      ...
      public function UiSlotmachineChat(param1:SymbolUiSlotmachineChatGeneric)
      {
         ...
         _messages = new Vector.<SlotmachineLogMessage>();
      }
      ...
      private function showNextReward() : void
      {
         ...
         var _loc3_:String = getMessage("#F0CC04",_messages[_currentIndex].message);
         _content.x = _right;
         _chatOutput.text = _loc3_;
         _chatOutput.x = 0;
         _animationRunning = true;
         _content.tweenTo(0.5,{"x":_normal});
         var _loc1_:int = 0;
         var _loc2_:Number = _chatOutput.textField.textWidth;
         if(_loc2_ > 408)
         {
            Tween.to(_chatOutput,3 + _loc1_,{
               "x":-(_loc2_ - 408),
               "delay":1
            });
         }
      ...
      private function handleRequests(param1:ActionRequestResponse) : void
      {
         ...
         switch(_loc5_)
         {
            case "getLastSlotmachineWins":
               ...
               {
                  _loc5_ = 0;
                  var _loc4_:* = param1.appResponse.win_messages;
                  for each(var _loc3_ in param1.appResponse.win_messages)
                  {
                     if(!messageInList(JsonUtil.decode(_loc3_)))
                     {
                        _loc2_ = new SlotmachineLogMessage(JsonUtil.decode(_loc3_));
                        if(_loc2_.message != "")
                        {
                           _messages.unshift(_loc2_);
                        }
                     }
                  }
                  if(_messages.length > 0 && _chatOutput.textField.text == LocText.current.text("dialog/tutorial/slotmachine/title"))
                  {
                     _content.tweenTo(0.5,{
                        "x":_left,
                        "delay":2,
      ...
      private function messageInList(param1:Object) : Boolean
      {
         var _loc2_:int = 0;
         _loc2_ = _messages.length - 1;
         while(_loc2_ >= 0)
         {
            if(_messages[_loc2_].equal(param1))
            {
               return true;
            }
            _loc2_--;
