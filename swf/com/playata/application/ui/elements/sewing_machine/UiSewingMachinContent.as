package com.playata.application.ui.elements.sewing_machine
{
   ...
   public class UiSewingMachinContent
   {
      ...
      private function onClickScrollDown(param1:InteractionEvent) : void
      {
         ...
         _currentScrollIndex = _currentScrollIndex + 5;
         if(_currentScrollIndex > _availablePatterns.length - 25)
         {
            _currentScrollIndex = int(Math.max(_availablePatterns.length - 25,5));
         }
         Environment.audio.playFX("ui_button_click.mp3");
         refreshPatterns();
      }
      ...
      private function refreshPatterns() : void
      {
         ...
         {
            _itemPattern[_loc1_].container.visible = _availablePatterns.length > _loc1_ + _currentScrollIndex;
            if(_itemPattern[_loc1_].container.visible)
            {
               _itemPattern[_loc1_].sewingMachinePattern = _availablePatterns[_loc1_ + _currentScrollIndex];
            }
            _loc1_++;
         }
         _btnScrollUp.visible = _availablePatterns.length > 25;
         _btnScrollDown.visible = _availablePatterns.length > 25;
         _btnScrollUp.buttonEnabled = _currentScrollIndex > 0;
         _btnScrollDown.buttonEnabled = _currentScrollIndex < Math.max(_availablePatterns.length - 25,5);
      }
