   ...
   public class UiPaymentOffer extends UiButton
   {
      ...
      public function refreshBonusInfo() : void
      {
         ...
         if(_enabled && savePercentage > 0)
         {
            ...
            if(isSpecialOffer)
            {
               _offerContent.savePercentage.y = -61;
               _offerContent.savePercentage.x = 46;
            }
      ...
      private function get savePercentage() : Number
      {
         ...
         try
         {
            ...
            if(_product.info is PremiumNormalBillingProductInfo)
            {
               _loc6_ = DialogPaymentBase.currentPaymentLocale;
               _loc3_ = DialogPaymentBase.currentPaymentMethod.baseOption;
               _loc5_ = Core.config.getTypedObject("payment_currencies").getTypedObject(_loc6_);
               _loc1_ = Number(_loc5_.getTypedObject("rates").getNumber(DialogPaymentBase.currentPaymentMethod.baseOption.toString()));
               _loc4_ = _product.info as PremiumNormalBillingProductInfo;
               _loc2_ = Number(getSavePercentage(_loc3_,_loc1_,_loc4_.premiumAmount,_product.price));
            }
            else if(_product.info is PremiumSpecialBillingProductInfo)
            {
               _loc7_ = _product.info as PremiumSpecialBillingProductInfo;
               _loc2_ = Number(getSavePercentage(_loc3_,_loc1_,_loc7_.premiumAmount,_product.price));
            }
            _savePercentage = _loc2_;
            var _loc10_:* = _savePercentage;
            return _loc10_;
            return;
         }
         catch(e:Error)
      ...
      private function getSavePercentage(param1:int, param2:Number, param3:int, param4:Number) : Number
      {
         if(param1 >= param3)
         {
            return 0;
         ...
         var _loc7_:Number = param4 * _loc8_;
         var _loc6_:Number = Math.round((1 - param2 / _loc7_) * -100);
         var _loc5_:String = DialogPaymentBase.currentPaymentLocale;
         if(_loc5_ == "pl_PL")
         {
            _loc6_ = NumberUtil.roundToNearest(10,_loc6_);
         }
         else
         {
            _loc6_ = NumberUtil.roundToNearest(5,_loc6_);
         }
         if(_loc6_ <= 0)
         {
            return 0;
