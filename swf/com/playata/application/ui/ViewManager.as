   ...
   public class ViewManager
   {
      ...
      public function setUser(param1:User, param2:Boolean = false) : void
      {
         ...
         else if(_user.character != null)
         {
            ...
            {
               _loggedIn = true;
               if(_user.character.activeQuest)
               {
                  showPanel("quest_progress");
               }
               else if(_user.character.activeConventionShow)
               {
                  showPanel("quest_progress");
               }
            ...
            else if(_activePanel != "quests" && _activePanel != "quest_progress" && _activePanel != "citymap" && _activePanel != "duels" && _activePanel != "battle" && _activePanel != "stat_quest" && _activePanel != "character" && _activePanel != "character_appearance" && _activePanel != "character_appearance_photographer" && _activePanel != "shop" && _activePanel != "work_new" && _activePanel != "dungeons" && _activePanel != "dungeon" && _activePanel != "movie_offers" && _activePanel != "movie_progress" && _activePanel != "slotmachine" && _activePanel != "guild")
            {
               showPanel("character");
            }
         }
      ...
      public function get titleBarPrefix() : String
      {
         ...
         var _loc1_:int = _user.character.totalNewMessages;
         if(_loc1_ == 0)
         {
            return "";
         }
      ...
      public function loadCharacter(param1:int, param2:Function) : void
      {
         if(param1 == -1)
         {
            return;
         ...
         Environment.application.sendActionRequest("getCharacter",{"character_id":param1},handleRequests);
      }
      
      public function loadGuild(param1:int, param2:Function) : void
      {
         _onLoadedGuild = param2;
         if(param1 == user.character.guildId)
         {
         ...
         Environment.application.sendActionRequest("getGuild",{"guild_id":param1},handleRequests);
      }
