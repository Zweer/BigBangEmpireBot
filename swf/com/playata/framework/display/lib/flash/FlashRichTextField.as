package com.playata.framework.display.lib.flash
{
   ...
   public class FlashRichTextField extends FlashTextField implements IRichTextField
   {
      ...
      private function convertElements() : void
      {
         ...
         var _loc2_:RichElementInfo = getNextElementInfo(_textField.text);
         while(_loc2_.index != -1)
         {
            _overlay.addElement(_loc2_);
            _textField.replaceText(_loc2_.index,_loc2_.index + _loc2_.code.length,"　");
            _loc1_ = new TextFormat();
            _loc1_.size = _defaultFontSize;
            _loc1_.letterSpacing = _loc2_.visual.width - 9;
            _textField.setTextFormat(_loc1_,_loc2_.index);
            _loc2_ = getNextElementInfo(_textField.text);
         }
         _overlay.refresh();
      }
