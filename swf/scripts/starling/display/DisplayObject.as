package starling.display
{
   ...
   public class DisplayObject extends EventDispatcher
   {
      ...
      public function drawToBitmapData(param1:BitmapData = null) : BitmapData
      {
         ...
         if(this is Stage)
         {
            _loc11_ = _loc6_.x < 0?-_loc6_.x / _loc4_:0;
            _loc12_ = _loc6_.y < 0?-_loc6_.y / _loc5_:0;
            if(!param1)
            {
               param1 = new BitmapData(_loc2_.backBufferWidth * _loc8_,_loc2_.backBufferHeight * _loc8_);
            }
         }
         else
         {
            _loc10_ = getBounds(_parent,sHelperRect);
            _loc11_ = _loc10_.x;
            _loc12_ = _loc10_.y;
            if(!param1)
            {
